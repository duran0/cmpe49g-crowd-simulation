...
methodology, experimental results, and analysis demonstrating that the
hybrid approach achieves a practical balance between computational
efficiency and adaptive accuracy.
...
dimensions 1.0 × 1.0 (normalized units). Agents are initialized on the
left side of the environment (x ∈ {[}0.02, 0.30{]}) with random vertical
positions. Two exits are placed at coordinates (1.0, 0.25) and (1.0,
0.75). Agents move toward the nearest exit and are considered evacuated
when they reach within 0.05 units of an exit.
...
Gaussian noise (σ = 0.15). The particle filter performs prediction,
...
2284 FPS at 100 agents to 1384 FPS at 500 agents. The Hybrid model shows
slightly lower but comparable performance, ranging from 2112 FPS at 100
agents to 1225 FPS at 500 agents.

The runtime difference between the two models remains relatively small.
At 500 agents, the Pure Perlin model completes 150 frames in 0.108
seconds, while the Hybrid model requires 0.122 seconds---approximately
13\% overhead. This overhead is attributed to the additional computation
required for critical zone detection, agent classification, and adaptive
field updates.

The Hybrid model tracks an average of 32-34\% of agents as critical
across the tested scenarios. For 500 agents, approximately 170 agents
are tracked at any given time, demonstrating that the correction layer
operates on a manageable subset rather than the full crowd.
...
average position RMSE of 0.243 units over 120 simulation frames. Figure
5 shows the RMSE trajectory over time. The filter maintains stable
tracking with RMSE fluctuating between 0.20 and 0.28 units,
demonstrating effective state estimation for small crowds.

However, the computational cost is substantial. The particle filter runs
at 564 FPS for 50 agents, compared to 2284 FPS for the Pure Perlin model
at 100 agents. Extrapolating this scaling behavior suggests that
applying a full particle filter to 300-500 agents would result in
prohibitively low frame rates for real-time applications.

This result validates the hybrid architecture's design choice: particle
filtering is accurate but expensive, making it suitable only for
localized application rather than full-crowd tracking.

\subsection{7.3 Evacuation Performance
Comparison}\label{evacuation-performance-comparison}

Table 1 (Figure 3) presents summary statistics across 5 runs with 200
agents each. The Pure Perlin model achieves an average evacuation time
of 5.25 ± 0.00 seconds and runtime of 0.08 ± 0.00 seconds. The Hybrid
Adaptive model shows identical evacuation times (5.25 ± 0.00 seconds)
with slightly higher runtime (0.09 ± 0.00 seconds).

Maximum density measurements reveal a subtle advantage for the Hybrid
model. The Pure Perlin model reaches peak densities of 1160 ± 108 agents
per unit area, while the Hybrid model achieves 1117 ± 64 agents per unit
area---approximately 4\% lower. This suggests that the adaptive
correction layer provides modest improvements in congestion management
near exits.

Figure 2 visualizes the evacuation time comparison. Both models show
similar average and maximum evacuation times, indicating that the Hybrid
model's adaptive behavior does not significantly delay or accelerate
overall evacuation in this simplified scenario. The primary benefit of
the Hybrid approach lies in its computational scalability and potential
for responding to dynamic events, rather than raw evacuation speed.

\subsection{7.4 Critical Agent Tracking}\label{critical-agent-tracking}

Figure 4 shows the number of tracked agents over time for the Hybrid
model with 300 total agents. The tracked count varies between 80 and 120
agents (27-40\% of the population), with peaks occurring when multiple
agents converge near sensor zones and exits.
...
tracking ratio across different crowd sizes (32-34\%) suggests that the
critical zone design scales appropriately with agent count.
...
\toprule\noalign{}
Model & Agents & Runtime (s) & FPS & Avg Evac Time (s) & Max Density \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Pure Perlin & 500 & 0.108 & 1384 & 5.25 & 2645 \\
Hybrid & 500 & 0.122 & 1225 & 5.25 & 2447 \\
Particle Filter & 50 & 0.213 & 564 & 4.20 & 311 \\
\end{longtable}
}

The Pure Perlin model excels in computational efficiency but lacks
adaptive response mechanisms. The Particle Filter provides accurate
state estimation but scales poorly. The Hybrid Adaptive model achieves
the intended balance: it maintains near-Perlin computational performance
while introducing adaptive behavior through selective correction, with
only 13\% runtime overhead at scale.
...
approximately 13\% computational overhead compared to Pure Perlin while
tracking 32-34\% of agents in critical zones. This overhead is
...
on the small-scale particle filter results.

The evacuation time results show no significant difference between Pure
Perlin and Hybrid models in the tested scenarios. This outcome has two
interpretations. First, in simple evacuation scenarios with no dynamic
obstacles or unexpected events, the goal-directed component of motion
dominates, and adaptive correction provides limited additional benefit.
Second, the feedback mechanism from the correction layer to the Perlin
field may require stronger coupling or more sophisticated adaptation
functions to produce measurable behavioral differences.

The density reduction observed in the Hybrid model (approximately 4\%
lower peak density) suggests that the correction layer does provide
subtle congestion management effects. Agents promoted to critical status
receive stronger goal bias and speed adjustments, allowing them to
navigate bottlenecks more efficiently. This effect is modest but
...
At 500 agents, the Hybrid model still achieves over 1200 FPS, far
exceeding typical real-time requirements (30-60 FPS for interactive
applications). This headroom allows for additional model complexity,
such as more sophisticated obstacle avoidance, social forces, or group
behaviors, without sacrificing real-time constraints.

The particle filter results highlight the importance of selective
application. While 100 particles suffice for accurate tracking of 50
agents, scaling to 500 agents would require thousands of particles to
maintain comparable accuracy, resulting in computational costs orders of
magnitude higher than the Hybrid approach. The Hybrid model's strategy
of applying intensive correction only to a subset of agents is therefore
not just beneficial but essential for large-scale scenarios.
...
maintains near-baseline computational performance (approximately 13\%
overhead) while introducing adaptive behavior through selective
correction of 32-34\% of agents.

The Pure Perlin model excels in raw efficiency, achieving over 2200 FPS
at 100 agents. The Particle Filter provides accurate state estimation
(RMSE ≈ 0.24) but scales poorly, running at only 564 FPS for 50 agents.
The Hybrid model occupies the intended middle ground, achieving over
1200 FPS at 500 agents while demonstrating modest improvements in
density management and maintaining the flexibility to respond to dynamic
events.
...
\def\labelenumi{\arabic{enumi}.}
\item
  A modular architecture that separates background coordination from
  local correction, allowing computational resources to be allocated
  where they are most needed.
\item
  Experimental validation showing that the hybrid approach achieves
  practical real-time performance across a range of crowd sizes.
\item
  Demonstration that selective application of intensive correction
  methods is essential for scalability---applying particle filtering to
  all agents results in prohibitive computational costs.
\end{enumerate}
...
quantification. Second, incorporating dynamic events such as obstacles,
hazards, or sudden crowd surges would better demonstrate the adaptive
capabilities of the framework. Third, extending the feedback mechanism
to include quantitative congestion estimates, uncertainty propagation,
or multi-scale field updates would strengthen the coupling between local
correction and global coordination.
...
In conclusion, the hybrid adaptive crowd simulation framework
demonstrates that it is possible to combine the efficiency of procedural
coordination methods with the adaptability of data-driven correction
techniques. This combination is well-suited for applications requiring
large-scale real-time crowds that can respond to local observations,
